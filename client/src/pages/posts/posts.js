        ...
        postsGrid.innerHTML = `
            <div class="no-posts">
                ...
                <div class="empty-submessage">
                    ${searchQuery ? 'Try different search terms or filters' : 'Be the first to share your thoughts!'}
                </div>
            </div>`;
        return;
    }
    ...
    postsGrid.innerHTML = postsToRender.map(post => `
        <div class="post-card" data-post-id="${post.id}">
            <div class="author-info">
                ...
                ` : `
                    <div class="author-avatar">${post.username ? post.username[0] : 'A'}</div>
                `}
                <div>
            ...
            <div class="post-meta">
                <span class="meta-item likes ${post.liked ? 'liked' : ''}" data-post-id="${post.id}">
...
async function createPost(title, content, tagIds) {
    try {
        ...
        const tagIdList = tagIds.split(',').filter(id => id).map(id => parseInt(id));
...
contentInput.addEventListener('input', () => updateCharCount(contentInput, contentCount));
...
function updateTagDropdown(filteredTags = availableTags) {
    ...
    if (!filteredTags || filteredTags.length === 0) {
        tagDropdown.innerHTML = '<div class="tag-option" style="color: rgba(255,255,255,0.5)">No matching tags</div>';
        return;
    }

    tagDropdown.innerHTML = filteredTags
        .filter(tag => !selectedTagIds.includes(tag.id))
        .map(tag => `
            <div class="tag-option" data-tag-id="${tag.id}" title="${tag.description || ''}">
                ${tag.name}
            </div>
        `).join('') || '<div class="tag-option" style="color: rgba(255,255,255,0.5)">No matching tags</div>';
}
